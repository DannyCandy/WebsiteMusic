...
import { AlertTriangle, Calendar, ChevronLeft, ChevronRight, Edit2, Filter, RefreshCw, Search, Trash2, X } from "lucide-react";
...
const SongsTable = () => {
	const { openUpdateSongDialog, openDeleteSongDialog } = useDialogStore();
	...
	const { data, isLoading, isError, isPlaceholderData, error, refetch, isFetching } = useSearchFilter<Song>( query, "/admin/songs");
	...
	return (
      	<div className="space-y-4">
			...
			<div className="flex items-center justify-between text-sm text-gray-400">
				<span>
					Showing {data?.docs.length} of {data?.totalDocs} songs
				</span>
				{filter && (
					...
					<TableBody>
						{
							...
							data?.docs.map((song) => (
								<TableRow key={song._id} className='hover:bg-zinc-800/50'>
									...
									<TableCell className='font-semibold'>{song.albumName ?? "Single"}</TableCell>
									<TableCell className='text-right'>
										<div className='flex gap-2 justify-end'>
											...
											<Button
												...
												onClick={() => openDeleteSongDialog(song._id)}
											>
